    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (error.message.includes('CORS') || error.message.includes('preflight') || error.message.includes('fetch')) {
                throw new APIError(
                    'Erro de CORS: Método OPTIONS não configurado',
                    0,
                    'CORS Error',
                    {
                        originalError: error.message,
                        solution: 'Configure CORS no AWS API Gateway para os métodos POST e OPTIONS'
                    }
                );
            }
        ...
        if (errorDetails) {
            if (details) {
                ...
                if (title.includes('CORS') || message.includes('CORS')) {
                    ...
                    detailsText += '3. Selecione o recurso /constellations\n';
                    detailsText += '4. Clique em "Actions" > "Enable CORS"\n';
                    detailsText += '5. Configure:\n';
                    detailsText += '   - Access-Control-Allow-Origin: *\n';
                    detailsText += '   - Access-Control-Allow-Headers: Content-Type,Authorization,X-Requested-With\n';
                    ...
                    detailsText += '\n⚠️ IMPORTANTE: Certifique-se de que o método OPTIONS foi criado!';
                }
